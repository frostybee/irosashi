...
static CODEGROUP_JS: &str = include_str!("../assets/js/codegroup.js");
...
mod tests {
    ...
    #[test]
    fn code_group_js_gated() {
        assert!(!generate(&Config::default()).contains(".kz-group"));
        let cfg = Config {
            code_groups: true,
            ..Default::default()
        };
        assert!(generate(&cfg).contains(".kz-group"));
    }
}
