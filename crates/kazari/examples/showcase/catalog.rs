...
fn engine(configure: impl FnOnce(KazariBuilder) -> KazariBuilder) -> Result<Kazari, Error> {
    let hl = iro::Highlighter::new().map_err(|e| Error::Highlight(e.to_string()))?;
    configure(
        Kazari::builder(hl)
...
pub fn build() -> Result<Catalog, Error> {
    ...
    let formats = category(
        ...
        vec![
            ...
            Ex::new("code-group", "Code Group (tabbed code blocks via markdown)", "Code group", code_group_html)
                ...
                .recipe("Rust", "let kz = Kazari::builder(hl).code_groups(true).build()?;\nlet html = kazari::markdown::render_markdown(&kz, markdown, pulldown_cmark::Options::empty())?;"),
    ...
    let svg = |code: &str,
               lang: &str,
               theme: &str,
               configure: &dyn Fn(&mut iro::SvgOptions)|
     -> Result<String, Error> {
        let mut options = iro::CodeToSvgOptions::new(lang, theme);
