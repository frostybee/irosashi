...
use irosashi::{CodeToHtmlOptions, CodeToTokensOptions, Highlighter, TokenizeOptions};
use irosashi_fidelity::alloc::{CountingAlloc, measure};
use irosashi_fidelity::bench_inputs::{SMALL, large, medium};
use irosashi_fidelity::highlighter;
...
fn warm_html(c: &mut Criterion) {
    ...
    for (lang, _, code) in SMALL {
        ...
        group.throughput(Throughput::Bytes(code.len() as u64));
        group.bench_with_input(BenchmarkId::new("irosashi", lang), &code, |b, code| {
            b.iter(|| black_box(h.code_to_html(code, &irosashi).unwrap()))
        });
...
fn warm_tokenize_line(c: &mut Criterion) {
    ...
    for (lang, grammar, _) in SMALL {
        ...
        let run = |session: &mut irosashi::Session| {
