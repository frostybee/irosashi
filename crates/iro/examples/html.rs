...
use iro::{CodeToHtmlOptions, DefaultColor, Highlighter, HtmlOptions};
...
fn main() -> Result<(), iro::Error> {
    ...
    let light_dark = highlighter.code_to_html(
        CODE,
        &CodeToHtmlOptions {
            tokens: iro::CodeToTokensOptions::new("rust", ""),
            themes: themes_ld,
            html: HtmlOptions {
...
<h2>1. Iro dialect, github-dark (single theme, always dark)</h2>
{single}
